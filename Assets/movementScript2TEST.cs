...
public class movementScript2TEST : MonoBehaviour {
...
//		r1Position + r2Position == rightPalPos; //change this to y value only?
	...
	void leftJump ()
	{
		//Debug.Log ("Help" + l1.GetComponent<Player1Script2TEST> ().jumped);
		if (l1.GetComponent<Player1Script2TEST> ().jumped == true && l2.GetComponent<Player2Script2TEST> ().jumped == true) {
	...
	void rightJump ()
	{
		if (r1.GetComponent<Player3Script2TEST> ().jumped == true && r2.GetComponent<Player4Script2TEST> ().jumped == true) {
			// if both l hitting jump, left jumping. bool called by pal script?
			//add bool to all player scripts called canJump?
			//
